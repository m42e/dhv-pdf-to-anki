...
def extract_questions_from_pdf(pdf_path, save_images=True, images_dir="question_images"):
    ...
    section_id = 0
    subsection_id = 0
    print(f"save_images: {save_images}, images_dir: {images_dir}")
    ...
    with fitz.open(pdf_path) as doc:
        ...
        for page in tqdm(doc, desc="Processing pages"):
            ...
            pdf_page_number = results[0] if results else ""
            ...
            lines = list(filter(lambda x: len(x) != 1, re.split(r"\n(?=(\d\d\d|\d\d|\d|[ABCD])\) +)", text)))
            for line in lines:
                    ...
                    for drawing in drawings:
                        if drawing["type"] == "fs":
                            ...
                            for bbox in answer_bbox:
                                if (
                                    rect.y0 - vertical_margin <= bbox.y0 <= rect.y1 + vertical_margin
                                    and rect.y0 - vertical_margin <= bbox.y1 <= rect.y1 + vertical_margin
                                ):
                                    if rect.x1 <= bbox.x0 and abs(rect.y0 - bbox.y0) < vertical_margin:
                                        _logger.debug(
                                            "Found a rectangle left of the answer: %s in bbox: %d, %d, %d, %d",
                                            rect,
                                            bbox.x0,
                                            bbox.y0,
                                            bbox.x1,
                        ...
                        if drawing["type"] == "f":
                            ...
                            if fill == (0.8429999947547913, 0.8429999947547913, 0.8429999947547913):
                                ...
                                for bbox in line_bbox:
                                    if rect.x0 <= bbox.x0 <= rect.x1 and rect.y0 <= bbox.y0 <= rect.y1:
        ...
        if save_images and current_question_data and answer_bboxes and question_bbox:
            save_question_image(
                doc, current_question_data, question_bbox, answer_bboxes, images_dir, current_question_data["page"]
            )
...
def save_question_image(doc, question_data, question_bbox, answer_bboxes, images_dir, page_num):
    ...
    try:
        ...
        if os.path.exists(image_path):
            _logger.debug(f"Image already exists: {image_filename}. Skipping saving.")
        ...
        _logger.debug(f"Saved image for question {question_id}: {image_path}")
